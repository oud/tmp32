...
public class PmEntrepriseServiceImpl implements PmEntrepriseService {
    ...
    /**
     *  Get all the pmEntreprises where Contrat is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<PmEntrepriseDTO> findAllWhereContratIsNull() {
        LOG.debug("Request to get all pmEntreprises where Contrat is null");
        return StreamSupport.stream(pmEntrepriseRepository.findAll().spliterator(), false)
            .filter(pmEntreprise -> pmEntreprise.getContrat() == null)
            .map(pmEntrepriseMapper::toDto)
