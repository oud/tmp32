...
export class GroupeComponent implements OnInit {
  ...
  protected readonly groupeService = inject(GroupeService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IGroupe): number => this.groupeService.getGroupeIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.groupes().length === 0) {
            ...
            this.groupes.set(this.refineData(this.groupes()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.groupes.set(this.refineData(dataFromBody));
  }
